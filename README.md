...
| POST | /api/batch-classify | 批量归类分析 |
| GET  | /api/review/due | 获取今日复习清单 |
| POST | /api/review/mark | 标记笔记复习状态 |

## 开发计划（路线图）

- [x] **v1.0 核心**：扫描索引、中文全文检索、笔记 CRUD、Web 界面、DeepSeek 归类/扩展/复习/问答
- [x] **v1.1 索引持久化**：索引落盘 + mtime 增量更新，加快大库冷启动
- [x] **v1.2 批量归类**：一键对全库分析归类、生成分类调整建议报告
- [x] **v1.3 双链与图**：笔记引用关系 / 主题聚类视图
- [x] **v1.4 复习计划**：按遗忘曲线生成每日复习清单
