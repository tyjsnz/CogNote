...
- **右键菜单**：新建/编辑/归类/重命名（笔记与分类）/删除/打开本地位置
- **拖拽移动**：目录树中直接拖动笔记/分类到目标分类；拖到笔记上即放入其子笔记目录
- **进行中提示**：移动/重命名等耗时操作显示 spinner 进度提示，完成或失败后反馈结果
...
- [x] **v1.9** 目录树拖拽移动：笔记/分类拖入目标分类，拖到笔记上放入其子笔记
- [x] **v1.10** 目录树体验：移动/重命名进行中提示、分类右键重命名（含同名笔记防呆）
