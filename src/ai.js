...
class DeepSeekAI {
  ...
  _system(kb) {
    return (
      `你是 Cognote 智能体，擅长对中文嵌入式开发笔记进行整理、归类、扩展与复习。` +
      (kb ? `\n当前 Cognote 收录主题包括：${kb}。回答使用 Markdown，内容专业、精炼、准确，不编造未提及的技术细节。` : '')
    );
  }
  ...
  async expandNote(doc, userPrompt) {
    const messages = [
      { role: 'system', content: this._system('嵌入式系统、Linux 驱动、计算机视觉、跟踪算法、充电桩等') },
      {
        role: 'user',
        content:
          `下面是 Cognote 中的一篇笔记。请结合 ${userPrompt || '相关领域知识'} 对这篇笔记进行知识扩展补全。\n\n` +
  ...
  async quickChat(notes, question) {
    ...
    const messages = [
      { role: 'system', content: this._system('嵌入式/驱动/算法/无人机') },
      {
        role: 'user',
        content:
          `基于 Cognote 笔记回答用户问题。引用时标注笔记来源。若笔记信息不足，诚实说明并给出搜索建议；不要编造。\n\n` +
